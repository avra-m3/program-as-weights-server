...
def program_id_for(
    ...
    The spec's identity (``name`` / ``slug``) is folded into the key so that
    two compiles of the same spec text under different identities are treated
    as distinct programs.

    A caller-supplied ``pseudo_program`` (POST /api/v1/compile/raw) is folded
    in too, so a raw compile with a custom pseudo-program never collides
    with a normal compile of the same spec text (whose pseudo-program is
    instead generated by the untrained pseudo compiler). It's only appended
    to the key material when given, so hashes for existing callers
    (``pseudo_program=None``) are unchanged.
    """
    parts = [compiler, spec, name or "", slug or ""]
    if pseudo_program is not None:
        parts.append(pseudo_program)
